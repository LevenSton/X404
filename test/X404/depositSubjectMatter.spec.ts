    ...
    context('Generic', function () {
        ...
        context('Negatives', function () {
            ...
            it('User should fail to deposit if not correct nft contract use safeTransferFrom.',   async function () {
                ...
                await expect(normalNft.connect(user)['safeTransferFrom(address,address,uint256,bytes)'](userAddress, x404Addr, 0, abicode)).to.be.revertedWithCustomError(x404, ERRORS.ErrorNFTAddress)
            });
            it('User should fail to deposit if use error param when safeTransferFrom.',   async function () {
                ...
                await expect(blueChipNft.connect(user)['safeTransferFrom(address,address,uint256,bytes)'](userAddress, x404Addr, 0, abicode)).to.be.revertedWithCustomError(x404, ERRORS.DeadLineInvaild)
            });
        })
