...
makeSuiteCleanRoom('redeemSubjectMatter', function () {
    ...
    context('Generic', function () {
        ...
        beforeEach(async function () {
            ...
            await expect(x404.connect(user).depositSubjectMatter([1], tomorrow)).to.be.not.reverted
        ...
        context('Negatives', function () {
            ...
            it('User should fail to redeem if tokenid length = 0.',   async function () {
                await expect(x404.connect(user).redeemSubjectMatter([])).to.be.revertedWithCustomError(x404, ERRORS.InvaildLength)
            });
            it('User should fail to redeem if erc20 token not enough.',   async function () {
                await expect(x404.connect(userTwo).redeemSubjectMatter([0])).to.be.reverted
            });
            it('User should fail to redeem if erc20 token not enough.',   async function () {
                const abicode = abiCoder.encode(['uint256'], [tomorrow]);
                ...
                await expect(x404.connect(userTwo).redeemSubjectMatter([0])).to.be.revertedWithCustomError(x404, ERRORS.NFTCannotRedeem)
            });
        })

        context('Scenarios', function () {
            it('Redeem success if you are the owner.',   async function () {
                await expect(x404.connect(user).redeemSubjectMatter([0])).to.be.not.reverted
                expect(await x404.connect(user).balanceOf(userAddress)).to.equal(parseEther("10000"))
                expect(await x404.connect(user).erc721BalanceOf(userAddress)).to.equal(1)
            ...
            it('Redeem success if you are the owner.',   async function () {
                ...
                await expect(x404.connect(userTwo).redeemSubjectMatter([0])).to.be.not.reverted
